...
class DeveloperServiceRepository {
    ...
    public function getBacklogsYear($id_area){
        ...
        if (!empty($user)) {
            ...
            echo '<script language="javascript">alert("No existen usuarios registrados");
            </script>'; 
    ...
    public function getBacklogs(){
        $backlogs =  DB::table('ss_backlog')->get();
        return $backlogs;
    }
    // Users
    public function getUsers($id_area){
     ...
     public function promedios($idBacklog, $idArea){ // POR BACKLOGS ACTUALES
       ...
       public function promedioBacklogAnterior($idBacklog , $idYear, $idArea)// comparacion ? Existe los backlos 
       { 
       ...
       if(!$anio){
        ...
        for ($i=0; $i < count($backlogs); $i++) { 
                    $promedioAnterior =  0;
                    $backlogAnterior = 'Backlog # ' . $idBacklog;
                    return compact('promedioAnterior', 'backlogAnterior');  
            }
        }
       }    
       
       public function promedioBacklogActual($idArea = 2){ // por backlog ultimo ingresado
   ...
   die();
        ...
        $backlogActualnombre = $this->backlogUtilizado($ultimoBacklogRegistrado[0]->ss_backlog_id, 2); // area tecnica
        $backlogPasadonombre = $this->backlogUtilizado($ultimoBacklogRegistrado[0]->ss_backlog_id - 1); // area soporte
        ...
        if($ultimoBacklogRegistrado[0]->ss_backlog_id - 1 > 0){
            ...
            return $newData;
          //  return $newData[0][0]->ss_backlog_id;      
        }
    }
  
}
       
       public function promedioPorBacklog($idBacklog, $idArea = 2){
     ...
     public function promedioGlobal($idArea){
        ...
        ->get();
             ...
             // PROMEDIO GLOBAL DE DESARROLLO AÑO ACTUAL
    public function promedioGlobalAnterior($idArea){
                $anio = DB::table('ss_year') // SUMATOIRA cmapos backlogs
                ->where('name', date('Y') - 1)
                ->get();
                if ($anio) { 
                   return "Backlogs no registrados";
                } else {
                        //promedio backlog actuañ
                        $promedioGLobal= DB::table('ss_backlog_year')
                        ->where('ss_area_id', $idArea)
