...
<tbody>
@for ($i = 0; $i < count($politicsSupport); $i++)
<tr>
<td> {{$politicsSupport[$i]->descripcion}} </td>
<td> {{$politicsSupport[$i]->type}} </td>
<td> {{$politicsSupport[$i]->points}} </td>
<td> 
<input type="checkbox" name="ss_politics_check[]" value="{{$politicsSupport[$i]->ss_politics_id}}"> </br>
